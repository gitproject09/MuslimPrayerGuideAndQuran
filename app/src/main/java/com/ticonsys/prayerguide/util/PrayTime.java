...
public class PrayTime {
  ...
  private double[] computeTimes(double[] times) {
    ...
    double Sehri = this.computeTime(
            180 - methodParams.get(this.getCalcMethod())[0], t[0]);

    double Fajr = this.computeTime(
        180 - methodParams.get(this.getCalcMethod())[0], t[1]);

    Log.d("fajrTimeis", ""+Fajr);

    double Sunrise = this.computeTime(180 - 0.833, t[2]);

    double Dhuhr = this.computeMidDay(t[3]);
    double Asr = this.computeAsr(1 + this.getAsrJuristic(), t[4]);
    double Sunset = this.computeTime(0.833, t[5]);

    double Maghrib = this.computeTime(
        methodParams.get(this.getCalcMethod())[2], t[6]);
    double Isha = this.computeTime(
        methodParams.get(this.getCalcMethod())[4], t[7]);

    double[] CTimes = {Sehri, Fajr, Sunrise, Dhuhr, Asr, Sunset, Maghrib, Isha};
  ...
  private ArrayList<String> computeDayTimes() {
    double[] times = {5, 5, 6, 12, 13, 18, 18, 18}; // default times fajr-5, sunrise-6, zuhor-12,
  ...
  private double[] adjustTimes(double[] times) {
    ...
    times[3] += this.getDhuhrMinutes() / 60; // Dhuhr
    if (methodParams.get(this.getCalcMethod())[1] == 1) // Maghrib
    {
      times[6] = times[5] + methodParams.get(this.getCalcMethod())[2] / 60;
    }
    if (methodParams.get(this.getCalcMethod())[3] == 1) // Isha
    {
      times[7] = times[6] + methodParams.get(this.getCalcMethod())[4] / 60;
    }
  ...
  private ArrayList<String> adjustTimesFormat(double[] times) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      Log.d("namTimes", ""+times[i]);

      if(i == 0){
        times[i] = (times[i] * 3600000) - (6 * 60000);
        times[i] = times[i] / 3600000;
        Log.d("HourMin", ""+times[i] );
      }
  ...
  private double[] adjustHighLatTimes(double[] times) {
    double nightTime = timeDiff(times[5], times[2]); // sunset to sunrise

    // Adjust Fajr
    double FajrDiff = nightPortion(methodParams.get(this.getCalcMethod())[0]) * nightTime;

    /*if (Double.isNaN(times[0]) || timeDiff(times[0], times[2]) > FajrDiff) {
      times[0] = times[2] - FajrDiff;

      Log.d("ChangeValue", "SehriVitore"+times[0]);
    }
    Log.d("ChangeValue", "SehriBaire"+times[0]);

    if (Double.isNaN(times[1]) || timeDiff(times[1], times[2]) > FajrDiff) {
      times[1] = times[2] - FajrDiff;
      Log.d("ChangeValue", "FazrVitore"+times[1]);
    }
    Log.d("ChangeValue", "FazrBaire"+times[1]);*/

    // Adjust Isha
    double IshaAngle = (methodParams.get(this.getCalcMethod())[3] == 0) ? methodParams.get(this.getCalcMethod())[4] : 18;
    double IshaDiff = this.nightPortion(IshaAngle) * nightTime;
    if (Double.isNaN(times[7]) || this.timeDiff(times[5], times[7]) > IshaDiff) {
    ...
    if (Double.isNaN(times[6]) || this.timeDiff(times[5], times[6]) > MaghribDiff) {
  ...
  public static void main(String[] args) {
    ...
    int[] offsets = {0, 0, 0, 0, 0, 0, 0, 0}; // {Sehri,Fajr,Sunrise,Dhuhr,Asr,Sunset,Maghrib,Isha}
